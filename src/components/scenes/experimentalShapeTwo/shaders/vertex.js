...
void main() {	
  ...
  float t = sin(localSpacePosition.x*localSpacePosition.y * 2.0 + uTime * 10.0)* cos(localSpacePosition.x*localSpacePosition.y * 8.0 + uTime * 12.0)*abs(uv.x-0.5);
  t = remap(t, -1.0, 1.0, 0.0, 0.7);
  float noiseOne = turbulenceFBM(localSpacePosition*1.0, 8, 0.5,2.0)*abs(uv.y-0.5);
...
//   noise =  remap(noise, -1.0, 1.0, 0.0, 1.0);
  ...
  float noise = cellular(localSpacePosition*1.0);
