...
void main() {	
  ...
  float t = cos(localSpacePosition.y * 10.0 + uTime * 10.0)* sin(localSpacePosition.x * 30.0 + uTime * 20.0);
  t = remap(t, -1.0, 1.0, 0.0, 0.7);
  localSpacePosition += normal * t;
  float noise = ridgedFBM(localSpacePosition, 4, 0.5, 2.0);
  float noiseOne = domainWarpingFBM(localSpacePosition/10.0);
//   noise =  remap(noise, -1.0, 1.0, 0.0, 1.0);
  ...
  vColour = mix(
      vec3(0.89788, 0.234, 0.67867),
      vec3(0.323, 0.786, 0.878),
      smoothstep(0.0, 0.9,noiseOne ));
} `;
