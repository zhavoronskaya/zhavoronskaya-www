...
const ExperimentalMaterialEight = shaderMaterial(
  {
    uTime: 0,
    uIterration: 4,
...
extend({ ExperimentalMaterialEight });
...
function Painting() {
  ...
  useFrame((state, delta) => {
    if (shaderRef.current) {
      shaderRef.current.uTime += delta * 0.4;
  ...
  return (
    <mesh>
      ...
      />
      <experimentalMaterialEight ref={shaderRef} wireframe={false} />
    </mesh>
  );
}
