...
void main() {	
  ...
  float t = cos(localSpacePosition.y*localSpacePosition.z*3.0 +uTime*2.0)*sin(localSpacePosition.x*10.0 +uTime*1.2)*atan(localSpacePosition.x,localSpacePosition.z);
  t = remap(t, -1.0, 1.0, 0.2, 0.9);
  float noiseOne = turbulenceFBM(localSpacePosition*12.0, 4, 0.5,2.0)*.5 + domainWarpingFBM(localSpacePosition*4.0)*0.2;

  // float cell = cellular(vec3(localSpacePosition*10.0))*0.3;
  // localSpacePosition += normal * cell;

  // localSpacePosition += normal * noiseOne;
  // localSpacePosition += normal * t;

//   noise =  remap(noise, -1.0, 1.0, 0.0, 1.0);
  // localSpacePosition += normal * noise;
  float cell = smoothstep(0.0,0.9, cellular(vec3(localSpacePosition.yyy*abs(sin(uTime*0.2)+1.0)))*10.0);
  localSpacePosition += normal * cell;
  localSpacePosition += normal * noiseOne;
