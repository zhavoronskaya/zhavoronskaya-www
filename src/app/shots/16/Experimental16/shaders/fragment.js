...
void main() {
  // float strength =0.1/setDistanceToCenter(gl_PointCoord, vec2(0.5)) - 0.1*2.0;
    ...
    strength =1.0 -  sdEgg(coord, 0.08,0.02 )*4.0;
    float glow = exp(-2.0 * setDistanceToCenter(gl_PointCoord, vec2(0.5)) / starRadius);

    vec3 hashValue = hash(vec3(coord, uTime*0.5));
float starBrightness = saturate(hashValue.z);
    float noiseSample = noise(vec3(coord, uTime * 1.5));
    float twinkleSize = (
        remap(noiseSample, -1.0, 1.0, 1.0, 0.1) * starRadius * 6.0);
    vec2 absDist = abs(coord - 0.5);
