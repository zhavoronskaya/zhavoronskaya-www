...
void main() {
    ...
    float t = sin(position.y/100.0)+cos(position.y);
    t = remap(t, -1.0, 1.0, 0.3, 0.7);
     ...
     float angleOffset = (PI*1.0 /(distanceToCenter)*44.4);
     angle += angleOffset;


    vec3 spitalPos = vec3(0.0);
    spitalPos = position;
     spitalPos.y += normal.y*-(sin(angle*PI*0.3+22.0*PI))*distanceToCenter;
    //  spitalPos.z += normal.z*(sin(angle*PI*0.3)+22.0)*distanceToCenter;
...
// spitalPos *=t;
    ...
    float cell = cellular(newPosition*30.0 +uTime)*0.062;
    newPosition += spiralNormal*cell;
  
    vDisplacement =cnoise(vec4(newPosition,uTime))*0.40;
