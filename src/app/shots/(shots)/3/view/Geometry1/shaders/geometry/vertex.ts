...
void main() {
  ...
  //deformation
  float t = domainWarpingFBM(modelPosition.xyz)+cnoise(vec4(modelPosition.xyz, uTime))+ sin(modelPosition.y*modelPosition.x*2.0 +uTime*2.0);
  t = remap(t, -1.0, 1.0, 0.0, 0.4);
  float noiseOne = ridgedFBM(modelPosition.xyz*1.0, 8, 0.5,2.0)*0.5;

  modelPosition.xyz += normal * noiseOne;
  modelPosition.xyz += normal * t;

  float elevationA1 = domainWarpingFBM(modelPositionA.xyz) +cnoise(vec4(modelPositionA.xyz, uTime))+ sin(modelPositionA.y*modelPositionA.x*2.0 +uTime*2.0);
  elevationA1 = remap(elevationA1, -1.0, 1.0, 0.0, 0.4);
  float elevationA2 = ridgedFBM(modelPositionA.xyz*1.0, 8, 0.5,2.0)*0.5;

  float elevationB1 = domainWarpingFBM(modelPositionB.xyz) +cnoise(vec4(modelPositionB.xyz, uTime))+ sin(modelPositionB.y*modelPositionB.x*2.0 +uTime*2.0);
  elevationB1 = remap(elevationB1, -1.0, 1.0, 0.0, 0.4);
  float elevationB2 = ridgedFBM(modelPositionB.xyz*1.0, 8, 0.5,2.0)*0.5;
...
vUv= uv;
vColor = mix(
      vec3(0.787788, 0.4534, 0.877867),
      vec3(0.18623, 0.8586, 0.338),
