...
void main() {	
  ...
  float t = domainWarpingFBM(localSpacePosition)+cnoise(vec4(localSpacePosition, uTime))+ sin(localSpacePosition.y*localSpacePosition.x*2.0 +uTime*2.0);
  t = remap(t, -1.0, 1.0, 0.0, 0.4);
  float noiseOne = ridgedFBM(localSpacePosition*1.0, 8, 0.5,2.0)*0.5;
...
//   noise =  remap(noise, -1.0, 1.0, 0.0, 1.0);
  ...
  vUv= uv;
  float distanceToCenter = length(localSpacePosition);
  vDistance= clamp(distanceToCenter,0.0,1.0);
  vColor = mix(
