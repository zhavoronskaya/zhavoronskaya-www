...
const ExperimentalTwentyMaterial = shaderMaterial(
  {
    uTime: 0,
    specMap: null,
  },
...
extend({ ExperimentalTwentyMaterial });
...
function Object() {
  ...
  const { nodes } = useGLTF("../../model//vase.glb");
  ...
  //       float vDisplacement =10.0*turbulenceFBM(objectNormal, 8, 0.5,2.0)+10.0*sin(objectNormal.x *objectNormal.z *8.0 + uTime*4.0)*cos(objectNormal.y *20.0 + uTime*1.0) ;
  //       // vDisplacement = remap(vDisplacement, -40.0, 40.0, 0.0, 20.0);
  ...
  return (
    <group>
      {/* <mesh>
        <sphereGeometry args={[100.0, 512, 512]} />
        <MeshTransmissionMaterial
          ref={transmissionRef}
          distortion={0.2}
          distortionScale={0.1}
          ...
          roughness={0.0}
          transmission={1.01}
          chromaticAberration={7.0}
          anisotropicBlur={10.3}
          background={"#02141c"}
          // anisotropy={1}
        />
      </mesh> */}

      <mesh geometry={nodes.vase.geometry} ref={ref}>
...
export default function Experience() {
  return (
    <>
      ...
      <color args={["#02141c"]} attach="background" />
      <ambientLight intensity={8.0} color={"#fff"} />
