...
const ParticlesMaterialTwentytwo = new shaderMaterial(
  { uSize: 20, uTime: 0 },
  pointsVertexShader,
  pointsFragmentShader
);

extend({ ParticlesMaterialTwentytwo });
...
function Object() {
  ...
  const { nodes } = useGLTF("../../model//shell.glb");
  ...
  return (
    <group rotation={[-Math.PI / 2, 0, 0]} ref={groupRef}>
      ...
      <Points limit={100000} ref={pointsRef}>
        <particlesMaterialTwentytwo
          ref={shaderRef}
          transparent={true}
          uSize={size}
          depthWrite={false}
        ...
        />
        {Array.from({
          length: nodes.shell.geometry.attributes.position.count,
        }).map((_, i) => {
          const i1 = i * 3;
          const position = [
            nodes.shell.geometry.attributes.position.array[i1],
            nodes.shell.geometry.attributes.position.array[i1 + 1],
            nodes.shell.geometry.attributes.position.array[i1 + 2],
          ];
