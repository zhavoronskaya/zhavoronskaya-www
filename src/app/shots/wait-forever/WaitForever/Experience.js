  ...
  React.useEffect(() => {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        vDisplacement = remap(vDisplacement, -20.0, 20.0, 0.0, 5.0);
...
function Logo({ url, ...props }) {
  const texture = useTexture("/image/wait.png");
  texture.encoding = THREE.sRGBEncoding;
  texture.flipY = true;
  const texture1 = useTexture("/image/forever.png");
  texture1.encoding = THREE.sRGBEncoding;
  texture1.flipY = true;
  const texture2 = useTexture("/image/or.png");
  texture2.encoding = THREE.sRGBEncoding;
  texture2.flipY = true;
  const texture3 = useTexture("/image/work.png");
  ...
  useFrame((state, delta) => {
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.uDelay.value += delta * 0.5;
      console.log(materialRef.current.uniforms.uDelay.value);
      if (audio) {
  ...
  return (
    <mesh ref={logo} rotation={[0, 0, 0]}>
      <planeGeometry ref={geomertyRef} args={[45, 7.5, 64, 32]} />
      <shaderMaterial
        ref={materialRef}
        // key="stable"
...
export default function Experience() {
  return (
    <>
      ...
      <color args={["#f7e1e9"]} attach="background" />
