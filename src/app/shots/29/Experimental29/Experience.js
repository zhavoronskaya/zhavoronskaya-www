...
const FBOParticles = () => {
  const shelly = useGLTF("../../model/shelly.glb");
  ...
  const simUniforms = React.useRef({
    positions: { value: makeTexture(shelly.scene.children[0].geometry) },
    positionsA: { value: makeTexture(shelly.scene.children[0].geometry) },
    positionsB: { value: makeTexture(star.scene.children[0].geometry) },
  ...
  return (
    <>
      <mesh position={[0, 0, 0]} geometry={shelly.scene.children[0].geometry}>
        <MeshTransmissionMaterial
          distortion={0.5}
          distortionScale={0.9}
          thickness={1.0}
  ...
  return (
    <>
      ...
      <GradientTexture
        ...
        colors={["aqua", "#07051f"]} // Colors need to match the number of stops
