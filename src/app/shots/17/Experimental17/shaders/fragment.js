...
void main() {
  ...
  //  coord = rotate2D(3.14159 * uTime*0.1) * coord;
    ...
    float strength =1.0 -  sdEgg(coord, 0.08,0.02 )*2.5;
...
float s1 = 1.0 -  sdEgg(coord + pixel.xy,0.08,0.02 )*2.5;

float s2 = 1.0 -  sdEgg(coord - pixel.xy, 0.08,0.02 )*2.5;

float s3 = 1.0 -  sdEgg(coord + pixel.yx, 0.08,0.02 )*2.5;

float s4 = 1.0 -  sdEgg(coord - pixel.yx, 0.08,0.02 )*2.5;
...
vec3 color = vColor;
   // vec3 color = mix( vColor, vec3(0.597,0.67,0.341), smoothstep(0.0,64.0, strength));
   // color= sinColorPalette(smoothstep(0.0,64.0, strength), vColor, vec3(0.597,0.67,0.341), vec3(0.5,0.5,0.5), vec3(0.489,0.2,0.943));
    vec3 lighting = diffuse;
